...
for num_iterations = 1:iterations
   y_current =  x2 * theta;
   grad_w= -(2/m) * sum(transpose(x2) .* (y - y_current));
   grad_b= -(2/m) * sum(y - y_current);

   %b_gradient = -(2/m) * sum(y - y_current);
   theta(1) = theta(1) - (alpha * grad_b);
   theta(2) = theta(2) - (alpha * grad_w);
